...
namespace Utilities.Text
{
	...
	{
		#region regex
		private const string CN_STRING = "^[\u4e00-\u9fa5]$";
		#endregion

		#region 拼音区编码数组
		private static readonly int[] pyFieldArray = new int[]
			{
		...
		#region 拼音数组
		private static readonly string[] pyArray = new string[]
			{
		...
		#region 二级汉字及拼音首字母数组
		private const string CHAR_FIRST_LETTER_L2 = "" +
		...
		private const string CHAR_L2 = "亍丌兀丐廿卅丕亘丞鬲孬噩丨禺丿匕乇夭爻卮氐囟胤馗毓睾鼗丶亟" +
		...
		public static string ToPinyin(this string hz)
		{
			var reg = new Regex(CN_STRING);//验证是否输入汉字
			var sb = new StringBuilder();
			var chars = hz.ToCharArray();//获取汉字对应的字符数组
			byte[] byteArr = new byte[2];
			int asc = 0, firstByte = 0, secondByte = 0;
			for (int j = 0; j < chars.Length; j++)
			{
				//如果输入的是汉字
				if (reg.IsMatch(chars[j].ToString()))
				{
					byteArr = System.Text.Encoding.Default.GetBytes(chars[j].ToString());
					firstByte = (short)(byteArr[0]);
					secondByte = (short)(byteArr[1]);
					asc = firstByte * 256 + secondByte - 65536;
					if (asc > 0 && asc < 160)
					{
						sb.Append(chars[j]);
					}
					else
					{
						...
						{
							case -9254:
								sb.Append("Zhen"); break;
							case -8985:
								sb.Append("Qian"); break;
							case -5463:
								sb.Append("Jia"); break;
							case -8274:
								sb.Append("Ge"); break;
							...
							case -3597:
								sb.Append("Die"); break;
							case -5659:
								sb.Append("Tian"); break;
							default:
								for (int i = (pyFieldArray.Length - 1); i >= 0; i--)
								{
									if (pyFieldArray[i] <= asc) //判断汉字的拼音区编码是否在指定范围内
									{
										sb.Append(pyArray[i]);//如果不超出范围则获取对应的拼音
										break;
									}
								}
								break;
				...
				{
					sb.Append(chars[j]);//如果不是汉字则返回
				}
			}
			return sb.ToString();//返回获取到的汉字拼音
		}
		...
		public static string GetFirstLetter(this string hz)
		{
			var sb = new StringBuilder();
			...
			{
				byteArr = System.Text.Encoding.Default.GetBytes(hz[i].ToString());
				...
				//一级汉字
				else if (byteArr[0] >= 176 && byteArr[0] <= 215)
				{
				...
				//二级汉字
				else if (byteArr[0] >= 215)
				{
					sb.Append(CHAR_FIRST_LETTER_L2.Substring(CHAR_L2.IndexOf(hz[i].ToString(), 0), 1));
				}
			}
			return sb.ToString();
		}
		...
		/// <returns>首字母</returns>
		public static string GetCodstring(this string hz)
		{
			var i = 0;
			ushort key = 0;
			var sb = new StringBuilder();
			...
			var unicodeBytes = unicode.GetBytes(hz);
			var gbkBytes = Encoding.Convert(unicode, gbk, unicodeBytes);
			while (i < gbkBytes.Length)
			{
				if (gbkBytes[i] <= 127)
				{
					sb.Append((char)gbkBytes[i]);
					i++;
				}
